...
namespace NullZoneTool
{
    public static class ConfigHelper
    {
        ...
        public static void DoLogin(string accountName, string token)
        {
            ...
            // Set only the account started through NullZone to Invisible.
            // This runs in the background so it does not delay or block Steam startup.
            SetInvisibleWhenSteamIsReady(steamExePath);
        }

        private static void SetInvisibleWhenSteamIsReady(string steamExePath)
        {
            _ = System.Threading.Tasks.Task.Run(async () =>
            {
                try
                {
                    // The modern Steam client creates steamwebhelper after its UI is ready.
                    // Wait for it instead of using a fixed long pause on every computer.
                    DateTime timeout = DateTime.UtcNow.AddSeconds(45);
                    while (DateTime.UtcNow < timeout)
                    {
                        bool steamIsRunning = Process.GetProcessesByName("steam").Length > 0;
                        bool uiIsRunning = Process.GetProcessesByName("steamwebhelper").Length > 0;

                        if (steamIsRunning && uiIsRunning)
                        {
                            break;
                        }
                    ...
                    // Give the account a few extra seconds to finish signing into Friends.
                    await System.Threading.Tasks.Task.Delay(7000).ConfigureAwait(false);

                    if (!File.Exists(steamExePath) || Process.GetProcessesByName("steam").Length == 0)
                    {
                        return;
                    }
                    ...
                    {
                        FileName = steamExePath,
                        Arguments = "steam://friends/status/invisible",
                        UseShellExecute = true
                    });
                }
                catch
                {
                    // Status switching is optional and must never interfere with login.
                }
            });
        }
